...
type FrameworkResultBandFormDialogProps = {
  errorMessage?: string
  initialValues?: FrameworkResultBandInput
  isOpen: boolean
  isSubmitting: boolean
  onClose: () => void
  onSubmit: (payload: FrameworkResultBandInput) => void
}

export function FrameworkResultBandFormDialog({
  errorMessage,
  initialValues,
  isOpen,
  isSubmitting,
  onClose,
  onSubmit,
}: FrameworkResultBandFormDialogProps) {
  ...
  const [fieldErrors, setFieldErrors] = useState<
    Partial<Record<'code' | 'label', string>>
  >({})
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!label.trim()) {
      errors.label = 'Nhãn không được để trống.'
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4 py-6">
      <section
        aria-labelledby="framework-result-band-form-title"
        aria-modal="true"
      ...
      >
        ...
        <div className="min-h-0 overflow-y-auto px-6 py-5">
          <form className="grid gap-5" onSubmit={handleSubmit}>
            ...
            <label className="grid gap-2 text-sm font-bold text-blue-950">
              <span className="whitespace-nowrap">Nhãn <span className="text-red-500">*</span></span>
              <input
                className={`h-11 rounded-lg border px-3 text-sm font-medium text-blue-950 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:bg-slate-50 ${fieldErrors.label ? 'border-red-500' : 'border-slate-200'}`}
                disabled={isSubmitting}
                onChange={(event) => {
